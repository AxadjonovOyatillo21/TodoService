...
from src.main_manager.todo_list_todo_operations import (
...
def todo_list_detail(list_public_id):
    ...
    todo_list_page = request.args.get("todo_list_page", 1, type=int)
    current_page = request.args.get("current_page", 1, type=int)
    print(request.args, current_page)
    todos = Todo.query.filter_by(owner=user).filter_by(
        todo_list=todo_list).order_by(Todo.id).all()
    todos = pagination.paginate_item(
        current_page,
        todos
    )
...
def update_todo_list(list_public_id):
    user = User.query.get(current_user.get_id())
    todo_list = TodoList.query.filter_by(
        public_id=list_public_id).first() or None
    
    data = request.get_json()
    response = update_operation(user, todo_list, data)
    return jsonify(response), response["code"]
